...
fun RegisterScreen(
    ...
    onRegisterNeedsLogin: (email: String) -> Unit,
    ...
    LaunchedEffect(uiState.registerSuccess, uiState.email) {
        if (uiState.registerSuccess) {
            snackbarHostState.showSnackbar(
                context.getString(R.string.auth_verify_email_sent, uiState.email)
            )
            onRegisterSuccess()
    ...
    LaunchedEffect(uiState.registerNeedsLoginEmail) {
        uiState.registerNeedsLoginEmail?.let { email ->
            snackbarHostState.showSnackbar(
                context.getString(R.string.auth_error_register_success_login)
            )
    ...
    Scaffold(snackbarHost = { SnackbarHost(snackbarHostState) }) { padding ->
        Column(
            ...
            OutlinedTextField(
                ...
                supportingText = {
                    Text(
                        uiState.passwordError ?: stringResource(R.string.auth_password_rules)
                    )
                },
                singleLine = true,
                enabled = !uiState.isLoading,
